...
module.exports = async function() {
  ...
  if (!Object.keys(genObj).length) {
    return this.console(`您还没有安装任何 generator，请先执行 install 命令安装`)
  }
  ...
  if (status !== 2) {
    ...
    if (needUpdate === '是') {
      await install.call(this, pkgName)
    }
  }
  this.yoemanEnv.register(
    path.join(this.dir.tpl, 'node_modules', pkgName),
    pkgName
  )
